...
public final class YamlLoader {
    ...
    static Path resolvePath(String yamlPath) {
        ...
        List<Path> candidates = List.of(
                directPath,
                Paths.get("src", "test", "resources", "browserstackExamples", fileName),
                directPath.resolveSibling(fileName + ".example"),
                Paths.get("src", "test", "resources", "browserstackExamples", fileName + ".example")
        );
        ...
        return candidates.stream()
                ...
                .orElseThrow(() -> new RuntimeException(
                        "Could not find BrowserStack YAML. Looked for: "
                                + candidates.stream().map(Path::toString).reduce((a, b) -> a + ", " + b).orElse(yamlPath)
                ));
    }
}
